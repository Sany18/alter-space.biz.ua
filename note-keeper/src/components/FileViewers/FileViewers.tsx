...
export const FileViewer: React.FC<Props> = () => {
  ...
  const { currentUser, isAuthInitializing, login } = useGoogleAuth();
  ...
  useEffect(() => {
    ...
    // Don't clobber an unsaved edit with stale remote content — a save retry (after
    // a background token refresh) is what should resolve this, not a re-fetch.
    if (activeFileInfo?.isFileChangedLocaly) return;
    if (!activeFileInfo?.error && activeFileContent !== null) return;

    const token = currentUser.googleAccessTokenToGD?.access_token;
    if (!token || isTokenExpired(currentUser.googleAccessTokenToGD)) return;
  ...
  useEffect(() => {
    if (currentUser.loggedIn && currentUser.needsReauth) {
      setMessage({
